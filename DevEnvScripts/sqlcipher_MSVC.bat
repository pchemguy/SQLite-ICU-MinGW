...
set ResultCode=!ErrorLevel!
if not "/!ResultCode!/"=="/0/" (
  echo SQLiteCipherSourceGet.bat error.
  exit /b !ResultCode!
)
if not exist "%BLDSQL%" (
...
set ResultCode=!ErrorLevel!
if not "/!ResultCode!/"=="/0/" (
  echo TCLTkGet.bat error.
  exit /b !ResultCode!
)
:: ICU
:: Requires icudtXX.dll icuucXX.dll icuinXX.dll
if not defined USE_ICU (set USE_ICU=1)
if %USE_ICU% EQU 1 (
  call "%~dp0ICUGet.bat"
  set ResultCode=!ErrorLevel!
  if not "/!ResultCode!/"=="/0/" (
    echo ICUGet.bat error.
    exit /b !ResultCode!
  )
  set ICUDIR=!ICU_ROOT!
  set ICUINCDIR=!ICU_INCLUDE!
...
if %USE_ZLIB% EQU 1 (
  ...
  set ResultCode=!ErrorLevel!
  if not "/!ResultCode!/"=="/0/" (
    echo ZLibGet.bat error.
    exit /b !ResultCode!
  )
  set ZLIBLIB=!ZLIB_LIBIMPORT!
  set ZLIBDIR=!ZLIB_ROOT!
  set ZLIBINCDIR=!ZLIB_INCLUDE!
  set ZLIBLIBDIR=!ZLIB_LIBPATH!
  set CCOPTS=%CCOPTS% !ZLIB_CFLAGS!
)
...
set FILENAME=%BLDSQL%\Makefile.msc
echo ========== Patching "%FILENAME%" ===========
set MATCH=  $(TOP)\ext\rtree\rtree.c \
set MATCH=%MATCH:\=\\\\%
sed -i "%FILENAME%" -e "/^%MATCH%$/r %FILENAME%.ext"
...
set FILENAME=%BLDSQL%\tsrc\main.c
echo ========== Patching "%FILENAME%" ===========
sed -i "%FILENAME%" -e "/^int sqlite3Fts5Init/r %FILENAME%.1.ext"
...
cd /d "%BLDSQL%\tsrc"
set FILENAME=sha1.c
sed -i "%FILENAME%" -e "s/\(hash_step_vformat\)/\1_sha1/"
...
if %USE_LIBSHELL% EQU 0 (
  copy tsrc\test_windirent.* .
  copy tsrc\fileio.c .
  set FILENAME=test_windirent.c
  echo ========== Patching "!FILENAME!" ===========
  call :EXPAND_INCLUDE "!FILENAME!" "test_windirent.h"
  (echo #include "test_windirent.c"
   echo #include "fileio.c" ) >>sqlite3.c
  call :EXPAND_INCLUDE "sqlite3.c" "test_windirent.c"
  call :EXPAND_INCLUDE "sqlite3.c" "fileio.c"
)
...
set FILENAME=%BLDSQL%\tsrc\zipfile.c
echo ========== Patching "%FILENAME%" ===========
sed -e "s/^static \(int zipfileRegister\)/\1/" ^
    -e "/^#include .sqlite3ext.h./i #ifdef %FLAG%\n" ^
    -i "%FILENAME%"
...
if "/%~2/"=="//" (
  set CommandText=echo %~1.c^| tr '[:upper:]' '[:lower:]'
  for /f "Usebackq delims=" %%G in (`!CommandText!`) do (set FILENAME=%%~G)
) else (set FILENAME=%~2)

echo ========== Patching "%FILENAME%" ===========
( echo #if defined^(%FLAG%^) & echo. & type "%FILENAME%"
  if exist "%FILENAME%.ext" type "%FILENAME%.ext"
  echo. & echo #endif /* defined^(%FLAG%^) */
) >>"%FILENAME%.tmp"
...
sed -e "s/^\(int SQLITE_CDECL\) main/\1 libshell_main/" ^
...
if %USE_ICU%  EQU 1 copy /Y "%ICU_BINPATH%\icu*.dll" .
if %USE_ZLIB% EQU 1 copy /Y "%ZLIB_BINPATH%\zlib1.dll" .
if /I "/%DBENG%/"=="/sqlcipher/" copy /Y "%OSSL_BINPATH%\lib*.dll" .
...
:EXPAND_INCLUDE
:: Call this sub with two arguments:
::   - %1 - Source file (may include path)
::   - %2 - Header file name (file name only)
::
setlocal
set "SOURCE=%~nx1"
set "HEADER=%~2"
...
set MATCH=^#\s*include\s*.%HEADER%.\s*$

sed -e "/%MATCH%/r %HEADER%.tmp" -e "/%MATCH%/d" -i "%SOURCE%"
del /Q "%HEADER%.tmp"
