...
#              6. State Preservation: Reads existing 'misc_ext_init.c' to 
#                 preserve previously registered extensions, allowing this script 
#                 to be called sequentially on different groups of files.
#
...
# Task 6: Read existing dispatcher to preserve previous registrations
proc load_existing_registry {outfile registry_var} {
    upvar 1 $registry_var registry
    ...
    if {[file exists $outfile]} {
        ...
        # Match ONLY the forward declarations: int sqlite3NameInit(sqlite3*);
        set pattern {int\s+sqlite3([A-Z][a-zA-Z0-9_]*?)Init\s*\(\s*sqlite3\s*\*\s*\)\s*;}
        
        foreach {- old_camel} [regexp -all -inline $pattern $content] {
            # Extra safety: never register the dispatcher itself
            if {$old_camel ne "ExtraAutoExt"} {
                set registry($old_camel) [camel_to_upper_snake $old_camel]
            }
        }
    }
}

# Task 7: Generate the dispatcher source file
...
# 1. Normalize input arguments to prevent NMake list evaluation bugs
set clean_files {}
foreach arg $argv {
...
set outfile "misc_ext_init.c"
array set module_registry {}

# 2. Load any previously registered modules to prevent overwrite
load_existing_registry $outfile module_registry

# 3. Process new files and merge into the registry (overwrites duplicates safely)
foreach extrasrc $clean_files {
    set module_data [process_module $extrasrc]
    
    if {[llength $module_data] == 2} {
        lassign $module_data mod_camel mod_upper
        set module_registry($mod_camel) $mod_upper
    }
}

# 4. Sort the registry alphabetically for deterministic code generation
set final_modules {}
foreach mod_camel [lsort [array names module_registry]] {
    lappend final_modules [list $mod_camel $module_registry($mod_camel)]
}

# 5. Generate the dispatcher if we have anything to write
if {[llength $final_modules] > 0} {
    generate_dispatcher $outfile $final_modules
}
